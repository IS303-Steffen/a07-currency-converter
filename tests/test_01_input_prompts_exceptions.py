...
from conftest import normalize_text, load_student_code, format_error_message, exception_message_for_students, get_similarity_feedback, default_module_to_test
import re

# Checks if the expected printed messages actually appear, but doesn't check for specific inputs or correct calculations.
def test_01_input_prompts_exceptions(current_test_name, input_test_cases):
    try:
