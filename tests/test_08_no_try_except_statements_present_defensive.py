...
def test_08_no_try_except_statements_present_defensive(current_test_name):
    try:
        ...
        student_code_path = f"{default_module_to_test_2}.py"  # Update with actual file name
        ...
        if forbidden_statements:
            formatted = format_error_message(
                    custom_message=(f"Your {default_module_to_test_2}.py file included at least one \"try\" statement, which the tests are "
                                    ...
                                    f"in the {default_module_to_test_2}.py file. Reach out to a TA or the professor if you're not sure how that is possible."),
