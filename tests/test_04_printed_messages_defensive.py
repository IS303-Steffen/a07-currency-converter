...
from conftest import normalize_text, load_student_code, format_error_message, exception_message_for_students, round_match, get_similarity_feedback, module_to_test_2
import re

# Checks if the expected printed messages actually appear, but doesn't check for specific inputs or correct calculations.
def test_04_printed_messages_defensive(current_test_name, input_test_cases):
    try:
        ...
        for input_test_case in input_test_cases:
            ...
            manager_payload = load_student_code(current_test_name, inputs, input_test_case, module_to_test=module_to_test_2)
            ...
            for expected_phrase in expected_printed_messages:
                ...
                # This converts any number that might have decimals into a consistently rounded number to avoid problems
                # with different rounding methods.
